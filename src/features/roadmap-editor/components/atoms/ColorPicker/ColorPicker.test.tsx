...
describe('ColorPicker', () => {
  ...
  it('전달된 색상 값을 사용한다', () => {
    const { container } = render(<ColorPicker value="#155dfc" />);
    const picker = container.querySelector('.react-colorful');
    expect(picker).toBeInTheDocument();
  });

  it('onChange 핸들러가 전달된다', () => {
    const handleChange = vi.fn();
    render(<ColorPicker value="#009689" onChange={handleChange} />);
    // react-colorful이 onChange prop을 받는지 확인
